...
class vector_series(series):
    ...
    def __init__(self, vec, first_date, name=None):
        ...
        for ndx,n in enumerate(vec):
            if n and (n != n):
                raise Exception('vector_series given NaN at {}'.format(jdate_to_text(ndx+first_date)))
            if not n:
...
class dateset(object):
    ...
    def __init__(self, vec, check_vector=True):
        self.vec = vec
        if check_vector:
            if type(vec) != type([]):
...
def dates(*specs, first=None, last=None, expanded=False, union=False):
    ...
    return dateset(result, check_vector=False)
